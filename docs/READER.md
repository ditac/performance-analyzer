...
to process events generated by the writer. Events are then processed into five second snapshots. Metrics are generated from these snapshots every five seconds and written to a new metricsDB. Old snapshot and db objects are deleted when their age crosses a threshold.

There are two main sub-components in the Metric Processor

* MetricsParser - This parses event files from shared memory and populates an inmemory database with the necessary data.

* MetricsEmitter - This stage queries the inmemory database for all the data and then populates the on-disk MetricDB.
...
Each subcomponent handles all known exceptions, logs them and continues processing. Unknown errors on the other hand will bubble up and cause the reader metrics processor to restart.

### Metrics Parser

The metrics parser parses files in shared memory and writes the data into inmemory snapshots in sqlite. We have multiple parsers and utility functions that make it easy to parse files and import data.
...
Snapshots are inmemory representation of data in sqlite tables. They make it easy for us to correlate data across different types of events. Snapshots can be deleted after all the corresponding metrics from the snapshot are written to the metrics database.

#### Sqlite

Sqlite is a very popular database that can be embedded into our process with minimal overhead. The memory footprint with sqlite and java is around 400mb for heavy workloads. The reader uses JDBC and JOOQ libraries to programmatically construct SQL statements and submit them to sqlite. These libraries helped us move fast and make the code more readable.
...
In some edge cases, the writer plugin might emit a start event but not an end event. For example, when Elasticsearch crashes and restarts. In order to handle such cases we take the following steps
...
* The metrics aggregation and processing logic is completely separate from the format of the inmemory snapshots. We dont expect any major changes to this code unless we add new features to the API.
